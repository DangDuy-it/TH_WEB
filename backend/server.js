...
//API: Cập nhật trạng thái của user (Active hoặc Banned)
app.put('/api/users/:user_id/status', (req, res)=>{
    const userId= req.params.user_id;
    const {status} = req.body; // truyền Active hoặc Banned cho status
    ...
    db.query(sql,[status, userId],(err, result)=>{
        ...
        //Sau khi cập nhật, lấy lại thông tin user mới nhất
